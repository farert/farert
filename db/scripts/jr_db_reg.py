...
def pad_list(org_list, pad_len, pad_value=''):
    """
    Padding the length of the list to the specified length.
    """
    new_list = list(org_list)
    while len(new_list) < pad_len:
        new_list.append(pad_value)

    return new_list[:pad_len]
...
#------------------------------------------------------------------------------
    ...
    # t_farels: 2島会社地方交通線
        param = list(map(lambda x: int(x.replace(',', '')) if x.replace(',', '').isdigit() else -1, linitems[1:8]))
        self.con.execute("""
insert into t_farels values(?, ?, ?, ?, ?, ?, ?, ?)""",
        [int(linitems[0].replace(',', '')), param[0], param[5], param[6], param[1], param[2], param[3], param[4]])
        # skm, ma, na, k, l, m, n
# a, 8, 5
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
    def reg_t_farespp(self, label, linitems, lin):
        ...
        if int(db_name) < 2019:
            for s in [ "あしかがフラワーパーク", "高輪ゲートウェイ", "JR総持寺" ]:
                if s in (station1[0], station2[0]):
                    return

        if 2023 <= int(db_name):
            for s in [ '動橋','粟津','小松','明峰','能美根上','小舞子','美川','加賀笠間','松任','野々市','西金沢' ]:
                if s in (station1[0], station2[0]):
                    return
