...
static const TCHAR *test_tbl[] = {
	...
	_T("大宮 上越新幹線 本庄早稲田"),                       // d		x	@高崎線, 
	_T("大宮 上越新幹線 高崎"),                             // e			@高崎線, 高崎, 
	_T("大宮 上越新幹線 上毛高原"),                         // f		x	@高崎線, 高崎, 
	_T("大宮 上越新幹線 浦佐"),                             // g 			@高崎線, 高崎, 上越線, 
	_T("大宮 上越新幹線 長岡"),                             // h			@高崎線, 高崎, 上越線, 宮内, 信越線(直江津-新潟), 
	_T("大宮 上越新幹線 燕三条"),                           // i		x	@高崎線, 高崎, 上越線, 宮内, 信越線(直江津-新潟), 
...
//}
				...
				// (xxは高崎かその上毛高原か本庄早稲田)
				//Route::NextShinkansenTransferTerm(bline_id, station_id2, station_id1);
				if (j_station_id == 0) {
					if (0 < zroute.size()) {
						if (32767 < zline.front()) {
							j_station_id = Route::NextShinkansenTransferTerm(ite->lineId, station_id1, station_id1n);
...
#if 0
TRACE(_T("\n;;%15s(%d)\t%s(%d)\n"), Route::LineName(route->back().lineId).c_str(), route->back().lineId, Route::StationName(route->back().stationId).c_str(), route->back().stationId);
 TRACE(_T("@"));
