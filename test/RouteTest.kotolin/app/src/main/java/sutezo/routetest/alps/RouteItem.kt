...
//	RouteItem
//
internal class RouteItem : Cloneable {
    constructor() {}
    ...
    // constructor
    constructor(lineId_: Int, stationId_: Int) {
        lineId = lineId_.toShort()
        stationId = stationId_.toShort()
        //    salesKm = fare = 0;

        //	if (lineId <= 0) {
        flag = RouteUtil.AttrOfStationId(stationId_) and RouteUtil.MASK_ROUTE_FLAG_SFLG
        //	} else {
    ...
    //
    constructor(lineId_: Int, stationId_: Int, flag_: Int) {
        //    salesKm = fare = 0;

        lineId = lineId_.toShort()
        stationId = stationId_.toShort()
        flag = RouteUtil.AttrOfStationId(stationId_) and RouteUtil.MASK_ROUTE_FLAG_SFLG
        flag = flag or (flag_ and RouteUtil.MASK_ROUTE_FLAG_LFLG)
    }

    constructor(ri: RouteItem) {
        let(ri)
    }

    // operator=
    fun let(ri: RouteItem) {
        lineId = ri.lineId
        stationId = ri.stationId
        flag = ri.flag
    }

    public override fun clone(): RouteItem? {
        try {
            val clone_obj = super.clone() as RouteItem
    ...
    fun refresh() {
        flag = RouteUtil.AttrOfStationId(stationId.toInt()) and RouteUtil.MASK_ROUTE_FLAG_SFLG
    }
