...
int Route::add(int line_id, int stationId1, int stationId2)
{
	...
	if (i == num) {		// ÅI•ªŠò‰w‚Å•œæ
		if (startStationId == stationId2) {	// type O/P
			rc = 1;		// type O/P
		} else {
			rc = inStation(startStationId, line_id, stationId1, stationId2);
			if (rc != 0) {
				rc = -1;	// •œæ
			} else {
				rc = 1;		// type O/P
	...
	/* ”­‰w/’…‰w‚Í”ñ•ªŠò‰w‚Ì‰Â”\«‚Ì‚ ‚é‚Ì‚Å”ñ•ªŠò‰w‘Î‰d•¡æÔƒ`ƒFƒbƒN */
		if (0 != (attrOfStationId(startStationId) & (1<<12))) {
			rc = 0;	/* ”­‰w=•ªŠò‰w‚È‚çƒ`ƒFƒbƒN‚³‚ê‚Ä‚¢‚éƒnƒY‚È‚Ì‚ÅOK */
		} else if (0 != (attrOfStationId(stationId2) & (1<<12))) {
			rc = 0;	/* ÅŒã(Current)‚Ì‰w=•ªŠò‰w */
		} else if (startStationId == stationId2) {
			rc = 1;	/* ’…‰w‚Åˆê•M‘‚«I—¹ */
		} else {
			/* ”­‰w=”ñ•ªŠò‰wAcurrent=”ñ•ªŠò‰wA”­‰w!=current */
			if (route_list.size() <= 1) {
				...
				if (endStationId <= 0) {
					rc = 0;				/* ‰‰ñ+’…‰w–¢w’è */
				} else if (endStationId == stationId2) {
					rc = 1;				/* ’…‰w‚Åˆê•M‘‚«I—¹(”­‰w=’…‰w->“üêŒ”) */
				} else {
					rc = inStation(endStationId, line_id, stationId1, stationId2);
				}
			} else {
				rc = inStation(stationId2, line_id, startStationId, route_list[1].stationId);
				if (rc != 0) {
					rc = -1;
				}// else {
				//	rc = 0;
