...
//
//	Copyright(c) sutezo9 2012 allrightreserved.
...
BEGIN_MESSAGE_MAP(Calps_mfcDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON_RULEAPPLY, &Calps_mfcDlg::OnBnClickedButtonRuleapply)
END_MESSAGE_MAP()
...
void Calps_mfcDlg::OnBnClickedButtonStartsel()
{
	...
	if (IDOK == dlg.DoModal()) {
		...
		if (startStationId != m_route.startStationId) {
			// 開始駅=終着駅は盲腸線でないことを確認
			if (m_route.endStationId  == m_route.startStationId) {
			...
			m_curStationId = m_route.startStationId = startStationId;		// 発駅Id
...
_T("着駅挿入、経路短縮"), 
			...
			int curLineId = LOWORD(pLRoute->GetItemData(numList - 1));	// last line
			ASSERT(0 < curLineId);
		...
		if (isComplete()) {
			...
			m_route.removeTail();
			if (2 <= numList) {
				m_curStationId = HIWORD(pLRoute->GetItemData(numList - 2));
			} else {
		...
		if (m_selMode != SEL_LINE) {
			...
			setupForStationlistByLine(LOWORD(pLRoute->GetItemData(numList - 1)), m_curStationId);
		}
	}
...
void Calps_mfcDlg::OnBnClickedButtonSel()
{
	...
	if ((pLSel->GetItemText(selIdx, 0))[0] == _T('-')) {
		SetDlgItemText(IDC_EDIT_STAT, 
_T("直前乗車路線は指定できません.戻って乗換駅を再選択してください."));
		return;
	}
	...
	if (m_selMode == SEL_LINE) {
		// add route list 路線Id
		pLRoute->SetItemText(pLRoute->InsertItem(LVIF_TEXT | LVIF_PARAM, nRoute, selTitle, 0, 0, 0, MAKELONG(selId, 0)), 1, _T(""));
...
#if 0	// into setupForStationlistByLine
...
#if 0	// 直前乗車路線は～で断られるので不要になった
		if (selId == m_route.startStationId) {
			SetDlgItemText(IDC_EDIT_STAT, 
	_T("同一駅の発着は経路を指定する必要があります."));
			return;
		}
#endif
		/* 経路追加 */// last line
		curLineId = LOWORD(pLRoute->GetItemData(nRoute - 1));	//(stationId is not yet strage in HIWORD)
		int rslt = m_route.add(curLineId, m_curStationId, selId);
					// selId == endStationIdの場合はrslt=0か1だが0で返すようにする？
		if (rslt < 0) {
			//MessageBox(_T("経路が重複しています."), _T("復乗エラー"), MB_ICONEXCLAMATION);
...
//<<<
			routeEnd();
		} else if (2 < rslt) {	// undefine
			ASSERT(FALSE);
			return;
		} else { /* rslt == 1 or 2 */
			if (rslt == 2) {	/* =2: OK(re-route) */
				nRoute = UpdateRouteList();					/* IDC_LIST_ROUTE update view */
				selId = HIWORD(pLRoute->GetItemData(nRoute - 1));		// last station
//>>↓にあった2行をここに持ってきてみた
				curLineId = LOWORD(pLRoute->GetItemData(nRoute - 1));	// last line
				ASSERT(0 < curLineId);
//<<
			}
#if 0	// into setupForLinelistByStation
...
//ここにあった2行は・・・↑
			...
			if ((m_selMode == SEL_TERMINATE) || (selId == m_route.endStationId)) {
				...
				pLRoute->SetItemData(nRoute - 1, MAKELONG(curLineId, selId));	//経路：分岐駅ID設定
...
void Calps_mfcDlg::OnBnClickedRadioBranchSel()
{
	...
	CListCtrl* pLRoute = reinterpret_cast<CListCtrl*>(GetDlgItem(IDC_LIST_ROUTE));
	int curLineId = LOWORD(pLRoute->GetItemData(pLRoute->GetItemCount() - 1));	// current select line

	ASSERT(((pLRoute->GetItemCount() <= 1) && (m_route.routeList().size() == 0) && 
			(m_curStationId == m_route.startStationId)) || 
			(m_curStationId == HIWORD(pLRoute->GetItemData(pLRoute->GetItemCount() - 2))));	// current select line
...
ASSERT(FALSE);
			ASSERT((m_curStationId == m_route.startStationId) && (m_curStationId != m_route.endStationId));
			ASSERT(numList == 0);
			return;
		}
#endif
		ASSERT(2 <= m_route.routeList().size());
		ASSERT((m_curStationId == m_route.endStationId) || (m_curStationId == HIWORD(pLRoute->GetItemData(numList - 1))));
		if (2 <= numList) {			/* Routeリストに分岐駅は2行以上? */
			m_curStationId = HIWORD(pLRoute->GetItemData(numList - 2)); // 最終行の分岐駅
		} else {
			m_curStationId = m_route.startStationId;		// 発駅
		}
		m_selMode = SEL_JUNCTION;
		...
		curLineId = LOWORD(pLRoute->GetItemData(numList - 1));	// last line
		setupForStationlistByLine(curLineId, m_curStationId);

		pLRoute->SetItemText(numList - 1, 1, _T(""));
		pLRoute->SetItemData(numList - 1, MAKELONG(curLineId, 0));
		...
		if (2 <= numList) {
			curLineId = LOWORD(pLRoute->GetItemData(numList - 2));	// last line
			ASSERT(0 < curLineId);
		} else {
			curLineId = 0;
...
//	IDC_BUTTON_RULEAPPLY [特例適用] button pushed
//
void Calps_mfcDlg::OnBnClickedButtonRuleapply()
{
	// TODO
}
...
void Calps_mfcDlg::setupForLinelistByStation(int stationId, int curLineId /* =0 */)
{
	...
	for (idx = 0; dbo->moveNext(); idx++) {
		...
		/* 前回選択路線は選択不可(行頭に"-") */
		lineName.Format(_T("%s%s"), (lineId == curLineId) ? _T("- ") : _T(""), dbo->getText(0));
...
int Calps_mfcDlg::UpdateRouteList()
{
	...
	for (idx = 0; pos != m_route.routeList().cend() ; pos++, idx++) {
		...
		if (stationId != m_route.endStationId) {
			stationName = m_route.stationName(stationId);
		}
		lineId = pos->lineId;
		lineName = m_route.lineName(lineId);

		pLRoute->SetItemText(
			pLRoute->InsertItem(LVIF_TEXT | LVIF_PARAM, 
								idx, 
								lineName, 0, 0, 0, 
								MAKELONG(lineId, stationId)), 
								1, stationName);
