...
//
RoutePass::RoutePass(const BYTE* jct_mask, SPECIFICFLAG last_flag, int32_t line_id, int32_t station_id1, int32_t station_id2)
{
...
//	@return 分岐点数
//
int32_t RoutePass::enum_junctions_of_line()
{
	int32_t c;
	static const char tsql[] = 
...
TODO
	...
	if (IS_LF_OSAKAKAN_PASS(_last_flag, LF_OSAKAKAN_NOPASS)) {
		...
		if (((0x01 & check()) == 0) || !BIT_CHK(_last_flag, BLF_OSAKAKAN_DETOUR)) {
			// a, c, e, f, g, h
			if ((0x01 & dir) == 0) {
		...
		if ((0x01 & (check() | rp.check())) == 0) {
			// a, e
			/* 両方向OK */
			_last_flag &= ~LF_OSAKAKAN_PASSMASK;
			_last_flag |= LF_OSAKAKAN_1PASS;
			_last_flag &= ~(1 << BLF_OSAKAKAN_1DIR);
		} else if ((0x01 & (check() ^ rp.check())) == 0) {
			// c, f, g
			_last_flag &= ~LF_OSAKAKAN_PASSMASK;
		...
		for (i = 0; i < 2; i++) {
			/* DB定義上の順廻り（内回り) : 外回り */
			jnum = ((0x01 & dir) == 0) ? enum_junctions_of_line() : 
		    	                enum_junctions_of_line_for_oskk_rev();
			if ((0x01 & check()) == 0) {
				break; /* OK */
			}
		    dir ^= 0x1;	/* 方向を逆にしてみる */
		...
		if (0 != k) { 
			...
			for (j = 0; j < 8; j++) {
				if ((k & (1 << j)) != 0) {
					...
					if (Route::Jct2id(jctid) == stationId2) {
						rc |= 2;   /* 終了駅 */
					} else if (Route::Jct2id(jctid) != stationId1) {
						rc |= 1;	/* 既に通過済み */
...
{
	if (Route::GetDistance(DbidOf::LineIdOf_OOSAKAKANJYOUSEN, station_id1, station_id2) <=
...
ASSERT(first_station_id1 = stationId1);
	...
	if (!BIT_CHK(last_flag, BLF_OSAKAKAN_2PASS) && 
	    (line_id != dbid.LineIdOf_OOSAKAKANJYOUSEN) &&
	    (0 != Route::InStationOnLine(line_id, stationId1))) {

	    BIT_ON(last_flag, BLF_OSAKAKAN_2PASS);
    }
...
ASSERT(FALSE);
			...
			///////////////////////////////////////////////////
			// calc fare
			if (!fare_info.calc_fare(route_list_raw, route_list_cooked)) {
				return tstring(_T(""));
			}
		} else {
			/* 経路変更（規則適用あり） */
...
//	   & 0x03 = 1 : 結果表示状態は{特定都区市内 -> 単駅} (「発駅を単駅に指定」と表示)
//	   & 0x03 =	2 : 結果表示状態は{単駅 -> 特定都区市内} (「着駅を単駅に指定」と表示)
//     & 0x0c = 4 : 大阪環状線1回通過(近回り)(規定)
//     & 0x0c = 8 : 大阪環状線1回通過(遠回り)
//
uint32_t Route::getFareOption()
{
	...
	// 大阪環状線 1回通過で近回り時
	if (((LF_OSAKAKAN_PASSMASK | (1 << BLF_OSAKAKAN_DETOUR)) & last_flag) == LF_OSAKAKAN_1PASS) {
	    rc |= 0x04;
    } else if (((LF_OSAKAKAN_PASSMASK | (1 << BLF_OSAKAKAN_DETOUR)) & last_flag) 
             == (LF_OSAKAKAN_1PASS | (1 << BLF_OSAKAKAN_DETOUR))) {
		rc |= 0x08;
	}
	return rc;
}
...
tstring Route::beginStationName(bool aplied_agree)
{
    ...
    if (!aplied_agree) {
		return Route::StationNameEx(route_list_raw.front().stationId);
    	...
    	if (sWork == _T("")) {
        	/* 単駅 */
        	return Route::StationNameEx(route_list_cooked.front().stationId);
	    } else {
    	    return sWork;
 		}
    }
...
tstring Route::endStationName(bool applied_agree)
{
	tstring sWork;

    if (!applied_agree) {
		return Route::StationNameEx(route_list_raw.back().stationId);
		...
		if (sWork == _T("")) {
			/* 単駅 */
			return Route::StationNameEx(route_list_cooked.back().stationId);
		} else {
			return sWork;
		}
	}
...
vector<int32_t> FARE_INFO::GetDistanceEx(int32_t line_id, int32_t station_id1, int32_t station_id2)
   int32_t FARE_INFO::aggregate_fare_info(int32_t line_id, int32_t station_id1, int32_t station_id2, int32_t station_id_0)
...
vector<int32_t> Route::GetDistance(int32_t b1lidflag, int32_t line_id, int32_t station_id1, int32_t station_id2)
{
	...
	sales_km_2 = Route::GetDistanceOfOsakaKanjyouRvrs(line_id, station_id1, station_id2);
...
//	@return 特定都区市内
//
tstring Route::CoreAreaNameByCityId(int32_t startEndFlg, int32_t flg, SPECIFICFLAG flags)
{
    int32_t city_idx = CoreAreaIDByCityId(startEndFlg, flg, flags);
    if (city_idx == 0) {
        return _T("");
    } else {
        return Route::CoreAreaCenterName(city_idx);
    }
}
...
//
vector<int32_t> Route::Get_route_distance(const vector<RouteItem>& route)
{
...
//
bool Route::CheckOfRule114j(const vector<RouteItem>& route, const vector<RouteItem>& routeSpecial, int32_t kind, int32_t* result)
{
	int32_t dkm;
	...
	if (!fi.calc_fare(route_work, route_work)) {
		ASSERT(FALSE);
		return false;					// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	} 
...
int32_t FARE_INFO::countOfFareStockDistount() const
{
	...
	switch (JR_GROUP_MASK & companymask) {
	case (1 << (JR_EAST - 1)):
		return 2;	/* JR EAST */
		break;
	case (1 << (JR_WEST - 1)):
	case (1 << (JR_CENTRAL - 1)):
		return 1;	/* JR WEST, JR_CENTRAL */
		break;
	default:
		break;
	}
...
//
int32_t FARE_INFO::aggregate_fare_info(const vector<RouteItem>& routeList_raw, const vector<RouteItem>& routeList_cooked)
{
...
//ASSERT((ite->flag) == 0);
		if (station_id1 != 0) {
			...
			if (ite->lineId == ID_L_RULE70) {
				...
				if (d.at(3) == -1) {		/* ite->stationId(station_id2)が境界駅の場合 */
...
//
bool FARE_INFO::calc_fare(const vector<RouteItem>& routeList_raw, const vector<RouteItem>& routeList_cooked)
{
	...
	if ((fare_add = FARE_INFO::aggregate_fare_info(routeList_raw, routeList_cooked)) < 0) {
		ASSERT(FALSE);
        reset();
		return false;
        //goto err;		/* >>>>>>>>>>>>>>>>>>> */
