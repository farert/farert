...
// ‰w‚Í•ªŠò‰w‚©
#define stationIsJunction(sid)	(0 != (Route::AttrOfStationId(sid) & (1<<12)))
...
RouteItem::RouteItem(IDENT lineId_, IDENT stationId_)
{
	...
	if (lineId <= 0) {
		flag = Route::AttrOfStationId((int)stationId_);
	} else {
...
" where %s=%d"
" and (l.lflg&((1<<23)|(1<<31)|(1<<24)|(1<<22)))=0"
...
//
//	@param [in] line_id1   i“ü˜Hü
//	@param [in] line_id2   ’Eo˜Hü
//	@param [out] node      ŒvZ—p˜HüA”­‰wAŠ‰w
//
	...
	if (num <= i) {	// •œæ‚È‚µ
		...
		if ((num - 1) == i) { /* last */
			if ((!stationIsJunction(stationId2)) ||
				((2 <= route_list_raw.size()) && (startStationId != stationId2) && (0 != Route::InStation(startStationId, line_id, stationId1, stationId2))) ||
				(((0 < endStationId) && (endStationId != stationId2) && (2 <= route_list_raw.size())) &&
				(0 != Route::InStation(endStationId, line_id, stationId1, stationId2)))) {
				rc = -1;	/* <k> <e> <r> <x> <u> <m> */
			} else  {
...
{
	const static TCHAR *coreAreaName[] = {  _T("“Œ‹“s‹æ“à[‹æ]"),
											_T("‰¡•ls“à[•l]"),
											_T("–¼ŒÃ‰®s“à[–¼]"),
											_T("‹“ss“à[‹]"),
											_T("‘åãs“à[ã]"),
	...
	if (((startEndFlg == CSTART) && BIT_CHK(flg, B1LID_BEGIN_YAMATE)) ||
		...
		return coreAreaName[NumOf(coreAreaName) - 1];	/* Rèü“à*/
	} else if (((startEndFlg == CSTART) && BIT_CHK(flg, B1LID_BEGIN_CITY)) || 
			   ((startEndFlg == CEND)   && BIT_CHK(flg, B1LID_FIN_CITY))) {
		flags &= 0x0f;
		if (flags < NumOf(coreAreaName)) {
			return coreAreaName[flags - 1];
		}
	}
	return _T("");
}
...
int Route::ReRouteRule70j(const vector<RouteItem>& in_route_list, vector<RouteItem>* out_route_list)
{
	...
	if (stage == 3) {	/* 70ğ“K—p‰w‚ğ’Ê‰ß */
		// “K‡
		vector<RouteItem> route_70list;
		Node node;	/* 70ğ’Ê‰ß: ˜HüA‰w1A‰w2 */
		if (!Route::Retrieve_70inout(route_item_1->lineId, route_item_2->lineId,
							  &node)) {
			ASSERT(FALSE);
			return -1;
		}
		if (node.stationId1 == node.stationId2) { /* —á‚¦‚Î‹—tü<->“Œ–kü */
...
bool Route::checkOfRuleSpecificCoreLine(int disCity, int* rule114)
{
	...
	n = Route::ReRouteRule69j(route_list_tmp, &route_list_tmp2);	/* 69ğ“K—p(route_list_tmp->route_list_tmp2) */
...
int Route::CheckOfRule88j(vector<RouteItem> *route)
{
	...
	if (!chk_distance1) {	/* chk_distance: _ŒË-•P˜HŠÔ‰c‹ÆƒLƒ */
	...
	if ((route->front().stationId == StationIdOf_OSAKA) && 
		...
		ASSERT(route->at(lastIndex - 1).lineId == LineIdOf_TOKAIDO);
		route->pop_back();
		return 2;
	}
	else {
...
bool Route::CheckOfRule114j(const vector<RouteItem>& route, const vector<RouteItem>& routeSpecial, int kind, int* result)
{
	...
	if ((kind & 1) != 0) {		/* ”­‰w‚ª“Á’è“s‹æs“à */
		...
		station_id1 = route.at(0).stationId;
		station_id2 = route.at(1).stationId;
	} else {
		ASSERT(FALSE);
		return false;					// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	}
...
#ifdef _DEBUG
	ASSERT(km_raw[0] == fi.sales_km);
	ASSERT(km_raw[1] == fi.calc_km);
#endif
 ...
 *	@return vector<int> [5] bit31=JRˆÈŠO‚Ì‰ïĞü^0=JRƒOƒ‹[ƒvĞü / IDENT1(‰w1‚Ìsflg) / IDENT2(‰w2‚Ìsflg(MSB=bit15œ‚­))
*/
...
"	(select sflg&8191 from t_station where rowid=?2) + (select sflg&8191 from t_station where rowid=?3) * 65536)"
	...
	if (ctx.isvalid()) {
		...
		if (ctx.moveNext()) {
			...
			result.push_back(ctx.getInt(5));	// bit31=JRˆÈŠO‚Ì‰ïĞü^0=JRƒOƒ‹[ƒvĞü / IDENT1(‰w1‚Ìsflg) / IDENT2(‰w2‚Ìsflg(MSB=bit15œ‚­))
		}
	}
	return result;
}
...
//		if ((fare_info.flag & MASK_TOKAIDO_SHINKANSEN) != 0) {
...
int Route::setup_route(LPCTSTR route_str)
{
	...
	for (p = _tcstok_s(rstr, token, &ctx); p; p = _tcstok_s(NULL, token, &ctx)) {
		if (stationId1 == 0) {
			startStationId = stationId1 = Route::GetStationId(p);
			if (stationId1 <= 0) {
