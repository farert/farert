...
//
//	@return DBO& iterator:int32_t 分岐ID
//
...
vector<int32_t> Route::Get_route_distance(const vector<RouteItem>& route)
{
	...
	while (it != route.cend()) {
		if (stationId != 0) {
			vector<int32_t> km;
			if (it->lineId == dbid.LineIdOf_OOSAKAKANJYOUSEN) {
...
TRACE(_T("******** loopRouteY **%s, %s******\n"), StationName(Jct2id(excNode1)).c_str(), StationName(Jct2id(excNode2)).c_str());
	...
	for (;;) {
		...
		for (i = 0; i < MAX_JCT; i++) {
			 ...
			 *  確定ノード番号更新する
			 */
			if ((doneNode < 0) || (!IsJctMask(i + 1) && (minCost[i] < minCost[doneNode]))) {
				doneNode = i;
			}
		}
		if (doneNode == -1) {
...
//   case 大阪環状線
//
vector<int32_t> FARE_INFO::GetDistanceEx(bool pass_osakakan_aggregate, int32_t line_id, int32_t station_id1, int32_t station_id2)
{
	vector<int32_t> result;
	int32_t rslt = 0;
...
//ASSERT((ite->flag) == 0);
		if (station_id1 != 0) {
			...
			int32_t b1lid_flag = // 計算集計時 大阪環状線 1回通ればON
			
			if (line_id == dbid.LineIdOf_OOSAKAKANJYOUSEN) {
				d = FARE_INFO::GetDistanceEx(pass_osakakan_aggregate, line_id, station_id1, station_id2);
				pass_osakakan_aggregate = true;
			} else {
				d = FARE_INFO::GetDistanceEx(line_id, station_id1, station_id2);
			}
