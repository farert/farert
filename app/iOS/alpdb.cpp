...
//////////////////////////////////////////////////////
#ifndef _WINDOWS
char* strcpy_s(char* dst, int maxlen, const char* src)
{
    int l;
    l = strlen(src);
    if (maxlen <= l) {
...
char* strcat_s(char* dst, int maxlen, const char* src)
{
    int l;
    int l2;
    l = strlen(src);
...
void conv_to_kana2hira(tstring& kana_str)
{
    ...
    for (pt = 0; pt < kana_str.size(); pt += cs) {
        ...
        if ((1 < cs) &&
            ...
            ASSERT(3 == cs);
            knum = kana_str[pt] << 16 | kana_str[pt + 1] << 8 | kana_str[pt + 2];
            knum -= u8"あ"[0] << 16 | u8"あ"[1] << 8 | u8"あ"[2];
            knum += u8"ア"[0] << 16 | u8"ア"[1] << 8 | u8"ア"[2];
            c[0] = knum >> 16 & 0xff;
            c[1] = knum >> 8 & 0xff;
            c[2] = knum & 0xff;
            c[3] = (char)0;
...
#else
		sqlite3_snprintf(sizeof(sql), sql, tsql,
								bKana ? "kana" : "name", sameName.c_str());
#endif
	}
	return DBS::getInstance()->compileSql(sql, true);
}
...
int Route::changeNeerest(bool useBulletTrain)
{
	...
	if (startNode == 0) { /* 開始駅は非分岐駅 */
		...
		if ((lastNode == 0) & (lid == Route::LineIdFromStationId(end_station_id))) {  /* 発駅と着駅は同一路線 (A) */
...
#ifndef _WINDOWS
//extern
void assertion_function(int cond)
{
    if (!cond) {
